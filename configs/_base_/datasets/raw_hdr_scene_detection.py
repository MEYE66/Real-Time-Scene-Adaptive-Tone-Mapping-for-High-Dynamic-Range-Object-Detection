...
train_dataloader = dict(  # 训练 dataloader 配置
    ...
    dataset=dict(  # 训练数据集的配置
        ...
        metainfo=metainfo,
        data_root=data_root,
        data_prefix=dict(img='RAWtiff'),  # 图片路径前缀
        # ann_file='annotations/scene/raw/day/train.json',  # 标注文件路径
        ann_file='annotations/scene/raw/night/train.json',  # 标注文件路径
...
val_dataloader = dict(  # 验证 dataloader 配置
    ...
    dataset=dict(
        ...
        data_root=data_root,
        data_prefix=dict(img='RAWtiff'),  # 图片路径前缀
        # ann_file='annotations/scene/raw/day/val.json',  # 标注文件路径
        ann_file='annotations/scene/raw/night/val.json',  # 标注文件路径
        test_mode=True,  # 开启测试模式，避免数据集过滤图片和标注
        pipeline=val_pipeline))

val_evaluator = dict(
    type='CocoMetric',
    # ann_file=data_root + 'annotations/scene/raw/day/val.json',
    ann_file=data_root + 'annotations/scene/raw/night/val.json',
    metric=['bbox'],
    format_only=False,
