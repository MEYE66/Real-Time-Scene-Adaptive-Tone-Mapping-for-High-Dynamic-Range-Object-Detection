# coding: utf-8
# File: pipeline.py
# Description: Execute ISP Pipeline
# Created: 2024-06-19  
# Author: Gongzhe Li
...
class Pipeline:
    ...
    def get_saturation_values(self):
        ...
        if self.cfg.hardware['rgb_bit_depth'] is not None:
            sdr_max_value = 2 ** self.cfg.hardware.rgb_bit_depth - 1
        else: # default 255 -8bit rgb
            sdr_max_value = 255
        ...
        if 'blc' in self.cfg.module_enable_status:
            ...
            hdr_max_gr = int(raw_max_value - blc.bl_gr + hdr_max_r * blc.alpha / raw_max_value)
            hdr_max_gb = int(raw_max_value - blc.bl_gb + hdr_max_b * blc.beta / raw_max_value)
    ...
    def get_output(self, data):
        ...
        if 'y_image' in data and 'cbcr_image' in data:
            ...
            output = output.astype(np.float32)
            output = output / self.cfg.saturation_values.hdr
            output = output ** (1/10)
            output = np.clip(255 * output, 0, 255).astype(np.uint8)
        else:
