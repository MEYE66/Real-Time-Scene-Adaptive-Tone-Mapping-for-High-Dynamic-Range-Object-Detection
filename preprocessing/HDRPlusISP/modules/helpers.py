...
def minmax_norm(image):
    out = (image - image.min())/(image.max()-image.min()).astype(np.float32)
    return out
...
def fGammaCompress_(x, threshold, gainMin, gainMax, exponent):
    ...
    # Check each element of x against the threshold
    mask = x <= threshold
    # Apply the gamma compression logic element-wise
    result[mask] = gainMin * x[mask]
    result[~mask] = gainMax * (x[~mask] ** exponent) - gainMax + 1.
...
def uGammaCompress_(x, threshold, gainMin, gainMax, exponent):
    ...
    # result = np.clip(result*(BIT24 - 1), 0, (BIT24 - 1)).astype(np.uint64)
    return result
