...
#     valid_files = files[:900]
#     # exit(234)
#     print(f'{len(valid_files)} annotations found')
#
#     l2c = Lableme2CoCo(img_postfix='.png') # FIXME: .npy  .png
#     instance = l2c.to_coco(valid_files)
#     l2c.save_coco_json(instance, path + '_train.json')


def main(args):
    path = os.path.realpath(args['path']).rstrip('/')
    assert os.path.isdir(path), f'Invalid path < {path} >!'
    dst_path = "/home/ligongzhe/data/annotations/scene/night/"

    json_path = glob(os.path.join(path, "*.json"))
    # print(len(json_path))
    night_json = []
    day_json = []

    for path in json_path:
        if path.startswith('/mnt/data1/RoD/00Anno/night'):
    ...
    l2c = Lableme2CoCo(img_postfix='.png')
    instance = l2c.to_coco(train_files)
    l2c.save_coco_json(instance, dst_path + '_train.json')


    l2c = Lableme2CoCo(img_postfix='.png')
    instance = l2c.to_coco(val_files)
    l2c.save_coco_json(instance, dst_path + '_val.json')
    return


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('-p', '--path', type=str, default='/mnt/data1/RoD/00Anno')
