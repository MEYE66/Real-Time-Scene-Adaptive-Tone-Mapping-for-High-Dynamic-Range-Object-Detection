...
class ResNet(BaseModule):
    ...
    def forward(self, x):
        ...
        # print(f"res_layers weight:{weight.sum() if weight is not None else 'None'} ")
        return tuple(outs)
...
from mmdet.models.backbones import tmo, ispnet
from arch.dirty_pixep import DiffISPNet


@MODELS.register_module()
class CANResNet(ResNet):
    def __init__(self, frozen_preprocessor=False, **kwargs):
        super(CANResNet, self).__init__(**kwargs)
        self.tone_mapper = tmo.DualCANTMO()
        state_dict = torch.load("/home/ligongzhe/ckpt/dualcan-00019.pt")['state_dict']
        self.tone_mapper.load_state_dict(state_dict)
        if frozen_preprocessor:
            self.tone_mapper.eval()
            for param in self.tone_mapper.parameters():
                param.requires_grad = False

    def forward(self, image_rgb):
        image_hsv = kornia.color.rgb_to_hsv(image_rgb)
        image_val = image_hsv[:, 2:3, :, :]
        out = self.tone_mapper(image_val, image_hsv)
        out = super(CANResNet, self).forward(out)
        return out
...
@MODELS.register_module()
class logSCANResNet(ResNet):
    def __init__(self, frozen_preprocessor=False, **kwargs):
        super(logSCANResNet, self).__init__(**kwargs)
        self.isp_preprocessor = tmo.logSCANTMO()
        state_dict = torch.load("/home/ligongzhe/ckpt/ada_canlog-00019.pt")['state_dict']
        self.isp_preprocessor.load_state_dict(state_dict, strict=False)
        if frozen_preprocessor:
            self.isp_preprocessor.eval()
    ...
    def forward(self, x):
        out = self.isp_preprocessor(x)
        out = super(logSCANResNet, self).forward(out)
        return out
...
@MODELS.register_module()
class ReinhardResNet(ResNet):
    def __init__(self, **kwargs):
        super(ReinhardResNet, self).__init__(**kwargs)
        self.isp_preprocessor = tmo.ReinhardTMO()
...
class RaodResNet(ResNet):
    def __init__(self, **kwargs):
        super(RaodResNet, self).__init__(**kwargs)
        self.isp_preprocessor = ispnet.RAODNet()

    def forward(self, x):
        out = self.isp_preprocessor(x)
        out = super(RaodResNet, self).forward(out)
        return out


@MODELS.register_module()
class IAResNet(ResNet):
    def __init__(self, **kwargs):
        super(IAResNet, self).__init__(**kwargs)
        self.isp_preprocessor = ispnet.IANet()

    def forward(self, x):
        out = self.isp_preprocessor(x)
        out = super(IAResNet, self).forward(out)
        return out
...
@MODELS.register_module()
class DiffISPResNet(ResNet):
    def __init__(self, **kwargs):
        super(DiffISPResNet, self).__init__(**kwargs)
        self.tone_mapper = DiffISPNet(alpha=1, sigma=1)

    def forward(self, image_rgb):
        out = self.tone_mapper(image_rgb, )
        out = super(DiffISPResNet, self).forward(out)
        return out
