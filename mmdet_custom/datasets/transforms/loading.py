...
def pack_raw_image(image_raw):
    ...
    im_out[:, :, 0] = image_raw[0::2, 0::2]
    im_out[:, :, 1] = image_raw[0::2, 1::2]
    im_out[:, :, 2] = image_raw[1::2, 0::2]
    im_out[:, :, 3] = image_raw[1::2, 1::2]

    return im_out


def flatten_raw_image(image_raw_4ch):
    """ unpack a 4-channel tensor into a single channel bayer image"""
    if isinstance(image_raw_4ch, np.ndarray):
        im_out = np.zeros((image_raw_4ch.shape[0] * 2, image_raw_4ch.shape[1] * 2, ), dtype=image_raw_4ch.dtype)
    elif isinstance(image_raw_4ch, torch.Tensor):
        im_out = torch.zeros((image_raw_4ch.shape[0] * 2, image_raw_4ch.shape[1] * 2, ), dtype=image_raw_4ch.dtype)
    else:
        raise Exception("Invalid input type. Expected numpy ndarray or torch Tensor.")

    im_out[0::2, 0::2,] = image_raw_4ch[:, :, 0]
    im_out[0::2, 1::2,] = image_raw_4ch[:, :, 1]
    im_out[1::2, 0::2,] = image_raw_4ch[:, :, 2]
    im_out[1::2, 1::2,] = image_raw_4ch[:, :, 3]
    return im_out


@TRANSFORMS.register_module()
class LoadImageFromNDRaw(BaseTransform):
    """ Load a hdr raw image from list of path
    Required keys:
        - img_path
    Modified Keys:
    - img
