...
data_root = '/home/gongzheli/data/RAW/'
...
train_pipeline = [  # 训练数据处理流程
    dict(type='LoadImageFromNDRaw', raw_type=True, norm=True, pack_raw=False),  # 第 1 个流程，从文件路径里加载图像。
    dict(
...
val_pipeline = [  # 测试数据处理流程
    ...
    dict(type='LoadImageFromNDRaw', raw_type=True, norm=True, pack_raw=False),  # 第 1 个流程，从文件路径里加载图像。
    # dict(type='CFATransform'),
    # dict(type='AWBTransform'),
    # dict(type='NormTransform'),
...
train_dataloader = dict(  # 训练 dataloader 配置
    ...
    dataset=dict(  # 训练数据集的配置
        ...
        metainfo=metainfo,
        data_root=data_root,
        data_prefix=dict(img='RAW'),  # 图片路径前缀
        ann_file='annotations/raw/train.json',  # 标注文件路径
...
val_dataloader = dict(  # 验证 dataloader 配置
    ...
    dataset=dict(
        ...
        data_root=data_root,
        data_prefix=dict(img='RAW'),  # 图片路径前缀
        # data_prefix=dict(img='RGBcs'),  # 图片路径前缀
        ann_file='annotations/raw/val.json',  # 标注文l件路径
        test_mode=True,  # 开启测试模式，避免数据集过滤图片和标注
        pipeline=val_pipeline))

val_evaluator = dict(
    type='CocoMetric',
    ann_file=data_root + 'annotations/raw/val.json',
    metric=['bbox'],
    format_only=False,
